<?php
/*
show_account_password_now.php - zmiana hasła użytkownika
*/
...
function FieldsControl($_newPassword, $_newPassword2) {
	$_fStatusError = 0;
	
	if(!((empty($_newPassword)) || (empty($_newPassword2))))
	{
...
function PasswordControl($_uLogin,$_newPassword2,$_host,$_db_user,$_db_password,$_db_name) 
{
	$_fStatusError = 0;
...
if(($_cError = FieldsControl($p_newPassword,$p_newPassword2)) == 0)
{
	if(($_cError = PasswordCompare($p_newPassword,$p_newPassword2)) == 0)
	{
		if(($_cError = PasswordControl($p_login,$p_newPassword2,$host,$db_user,$db_password,$db_name)) == 0)
		{
