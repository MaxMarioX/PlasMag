...
function adUpdateCustomer($_CustomerId, $_CustomerName,$_CustomerNip,$_CustomerRegon,$_host,$_db_user,$_db_password,$_db_name) 
{
	$_fStatusError = 0;
	
	if($ConnectNow = mysqli_connect($_host,$_db_user,$_db_password,$_db_name))
	{	
		if((mysqli_query($ConnectNow,"UPDATE klienci SET nip='".$_CustomerNip."', regon='".$_CustomerRegon."', nazwa='".$_CustomerName."' WHERE id_klient='".$_CustomerId."';")) == NULL)
		{
			$_fStatusError = 2;
		}			
	$ConnectNow->close();
...
{
	if(($_cError = adUpdateCustomer($CustomerId,$CustomerName,$CustomerNip,$CustomerRegon,$host,$db_user,$db_password,$db_name)) == 0)
	{
		
		$Result = "Dane klienta ".$CustomerName." zostały zaktualizowane!";
	}
	else {
		if($_cError == 1)
